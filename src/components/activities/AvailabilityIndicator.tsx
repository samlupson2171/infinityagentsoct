import React from 'react';
// Define IActivity interface locally to avoid importing mongoose models on client
interface IActivity {
  _id: string;
  name: string;
  category: string;
  location: string;
  pricePerPerson: number;
  minPersons: number;
  maxPersons: number;
  availableFrom: string;
  availableTo: string;
  duration: string;
  description: string;
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
}
import { getActivityAvailabilityStatus } from '@/lib/availability-utils';

interface AvailabilityIndicatorProps {
  activity: IActivity;
  showMessage?: boolean;
  className?: string;
}

export default function AvailabilityIndicator({
  activity,
  showMessage = true,
  className = '',
}: AvailabilityIndicatorProps) {
  const {
    status,
    message,
    className: statusClassName,
  } = getActivityAvailabilityStatus(activity);

  const getIcon = () => {
    switch (status) {
      case 'available':
        return (
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
              clipRule="evenodd"
            />
          </svg>
        );
      case 'expired':
        return (
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
              clipRule="evenodd"
            />
          </svg>
        );
      case 'inactive':
        return (
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M13.477 14.89A6 6 0 015.11 6.524l8.367 8.368zm1.414-1.414L6.524 5.11a6 6 0 008.367 8.367zM18 10a8 8 0 11-16 0 8 8 0 0116 0z"
              clipRule="evenodd"
            />
          </svg>
        );
      case 'upcoming':
        return (
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"
              clipRule="evenodd"
            />
          </svg>
        );
      case 'expiring-soon':
        return (
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
              clipRule="evenodd"
            />
          </svg>
        );
      default:
        return null;
    }
  };

  return (
    <div className={`inline-flex items-center ${className}`}>
      <span
        className={`inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full ${statusClassName}`}
      >
        {getIcon()}
        {showMessage && <span className="ml-1">{message}</span>}
      </span>
    </div>
  );
}

// Compact version for use in tables or lists
export function AvailabilityBadge({
  activity,
  className = '',
}: {
  activity: IActivity;
  className?: string;
}) {
  return (
    <AvailabilityIndicator
      activity={activity}
      showMessage={false}
      className={className}
    />
  );
}

// Full version with detailed message
export function AvailabilityStatus({
  activity,
  className = '',
}: {
  activity: IActivity;
  className?: string;
}) {
  return (
    <AvailabilityIndicator
      activity={activity}
      showMessage={true}
      className={className}
    />
  );
}
